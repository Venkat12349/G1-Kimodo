...
class ProtoMotionsTrackerPolicy(Policy):
    ...
    def post_step_callback(self, commands=None):
        ...
        for cmd in commands or []:
            if cmd in ("[MOTION_RESET]", "[MOTION_FADE_IN]"):
                logger.info(
                    f"[TrackerPolicy] RESET: frame={self._frame}, total={self._player.total_frames}"
                )
                self.reset()
                reset_happened = True

        # Only advance frame if no reset happened this tick
        if not reset_happened and not self._paused and not self._default_pose_mode:
            self._frame += 1
            if self._frame >= self._player.total_frames:
                self._frame = self._player.total_frames - 1
                self._motion_done = True
